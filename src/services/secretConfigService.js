...
class SecretConfigService {
  constructor() {
    if (!process.env.JWT_SECRET_NAME_POSTFIX || !process.env.MONGODB_URI_NAME_POSTFIX) {
      logger.error("Environment variable JWT_SECRET_NAME_POSTFIX or MONGODB_URI_NAME_POSTFIX is not set");
      process.exit(1);
    }
    ...
    this.secretConfig.addSecret(
      "mongodbUri",
      process.env.MONGODB_URI_NAME_POSTFIX
    );
  }
